...
function rebuildDb(connection, silent) {
  ...
  return Promise.resolve().then(function () {
    return knex.raw('DROP DATABASE ' + dbName).then(function () {
      return logger.log('Dropped ' + dbName + ' database', silent);
    }).catch(function (err) {
    ...
    return logger.log('Created ' + dbName + ' database', silent);
  }).then(function () {
...
function init() {
  try {
    ...
    _child_process2.default.exec('npm install --save cambio', function (error, stdout, stderr) {
      if (error) {
